...
def test_get_all_lots_success(mock_get_db):
    """
    SCÉNARIO: On demande la liste de tous les lots.
    ...
    # Simuler la chaîne SQLAlchemy: session.query().options().order_by().all()
...
def test_get_lot_by_id_success(mock_get_db):
    """
    SCÉNARIO: On demande un lot spécifique avec un ID valide.
...
def test_get_lot_by_id_not_found(mock_get_db):
    """
    SCÉNARIO: On demande un lot qui n'existe pas.
    QUAND: get_lot_by_id est appelée.
...
def test_update_lot_success(mock_get_db, mock_commit_session):
    """
    SCÉNARIO: On met à jour les informations d'un lot.
...
def test_get_alertes_by_lot_success(mock_get_db):
    """
    SCÉNARIO: On demande l'historique des alertes pour un lot.
...
def test_update_lot_status_changes_status_to_perime(mock_get_db, mock_commit):
    """
    SCÉNARIO: Un lot est stocké depuis plus de 365 jours.
    ...
    mock_lot.datSto = datetime.now(UTC) - timedelta(days=400) # Périmé
    # Simuler la hiérarchie pour que la 2e partie de _calculer_statut_lot ne soit pas exécutée
    mock_lot.entrepot.exploitation.pays = None

    # La requête doit retourner ce lot
    ...
    # --- ASSERT ---
    # Le statut a bien été modifié sur l'objet mock
    assert mock_lot.statut == StatutLot.PERIME.value
    # La session a été commit
